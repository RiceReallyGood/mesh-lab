...
**按真实执行顺序排**（唤醒 → 读取解码 → handler → 编码 → 发送）：

| 阶段 | 直连 | 单跳 | 双跳 |
|---|---:|---:|---:|
| epoll 唤醒 → 开始读 | 1.1 µs | 1.0 µs | 950 ns |
| 取首字节（Peek） | 320 ns | 300 ns | 280 ns |
| TTHeader 解码 | 2.2 µs | 2.0 µs | 1.7 µs |
| payload 反序列化 | 660 ns | 620 ns | 520 ns |
| 　└ 读取+解码（整段） | 6.5 µs | 5.9 µs | 5.1 µs |
| **业务 handler** | **240 ns** | **240 ns** | **220 ns** |
| 编码（发送前） | 1.8 µs | 1.8 µs | 1.6 µs |
| 　└ 其中 payload 编码 | 500 ns | 500 ns | 470 ns |
| 写 socket | 7.3 µs | 7.4 µs | **5.7 µs** |
| 总时长 | 23.2 µs | 21.3 µs | **17.7 µs** |

> **三个区间在 2026-08-10 改过名**，数值未变，见 [probe-points.md](probe-points.md)
> 「三个名字骗人的区间」：
> 「等待请求体」→ **payload 反序列化**（它包住的是 `unmarshalThriftData`，是解码不是等待）；
> 服务端的「等待对端(纯网络)」→ **取首字节(Peek)**（数据已在 LinkBuffer，此处无网络等待）；
> 「payload 解码」→ **payload 编码**（`mesh_payload_codec_*` 只插在发送路径）。
...
| 协议层解消息头 | 490 ns | 910 ns |
| 路由匹配 | 470 ns | 860 ns |
| 取上游连接 | 1.6 µs | 3.5 µs |
| 帧编码 | 4.8 µs | 7.5 µs |
| 写 socket（仅入队） | 290 ns | 710 ns |
| ★等待上游 | 181.1 µs | 55.5 µs |
| 响应解码 | 4.2 µs | 7.0 µs |
| **自身合计** | **15.9 µs** | **27.6 µs** |

> **2026-08-10 起 merge 的 detail 标签统一了格式**，去掉了 ①→② 这类带圈编号
> （四个节点里只有 Envoy 有，其余节点没有对应物，混排反而难读），改用统一的
> 树形缩进表达包含关系。**点位的 ①–⑧ 编号仍是权威口径，见
> [probe-points.md](probe-points.md) §一** —— 只是不再出现在分析输出的行首。
>
> 同一轮还发现「写 socket」只是入队（真正的 writev 在事件循环稍后执行），
> 标签已改名。上表的 290 ns / 710 ns 就是入队时间，**不是系统调用**。
