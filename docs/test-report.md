...
> ⚠️ **2026-08-10 修正过三次归因口径，本报告的数字用的是旧口径。**
> 级差与对照类结论不受影响，绝对分解（「往返占多少 / 节点自身占多少」）需按
> **§2.6** 的方向校正。原始数据未入库无法重算，故以说明代替重跑。
...
### 2.6 归因口径在 2026-08-10 改过三次，**本报告的数字用的是旧口径**

§3.2 把「自身处理」定义为 `节点总时长 − 该节点的等待时间`。这个定义看着无害，
但它依赖两个量：**节点区间的边界**、以及**「等待」到哪里为止**。
两者在 2026-08-10 各修正过，方向完全一致 ——
**把本属于某个节点、此前测不到或被误算成「等待」的时间，从「往返」搬回「自身」。**

本报告的数据无法重算（2026-08-07 的原始 `.ndjson` 未入库），所以下面说清楚
改了什么、影响多大、往哪个方向偏，读者据此校正即可。

#### 变动一：服务端 netpoll 收包此前完全不可见

服务端最早只能看到 `mesh_netpoll_onread`（`OnRead` 入口）。它**之前**的
epoll 唤醒 → readv → LinkBuffer → goroutine 调度全无点位 —— 而不可见的时间
不会消失，它落进了 `envoy-in ↔ server` 的往返里。

补齐后实测这一段 **12.9 µs**（readv 4.7 µs + goroutine 调度 7.8 µs 是大头）。

> 影响：**§3.2 的 `server 自身` 被低估**，§3.4 的 `envoy-in 总 − server 总`
> （65.4 µs）里有约 13 µs 其实是服务端自己的收包，不是 UDS 往返。
...
| 被误算的 | 实测量级 | 实际属于 |
|---|---:|---|
| client 的「取数据」（poller 派发 + readv + goroutine 调度） | 5.9 µs | client 自身 |
| Envoy 的**异步写出**（事件循环调度 + writev） | ~7 µs | envoy 自身 |

第二项尤其反直觉：Envoy 的 `write()` 只做 `write_buffer_->move()` + 激活写事件，
**真正的 writev 由事件循环在 `up_write_done` 之后才执行** —— 落在「等待上游」
窗口的开头。名字叫等待，其实本跳还在发。

现在两侧的「等待」都只算到 **epoll 就绪**为止。

> 影响：**§3.2 的 `client 自身` 与 `envoy-* 自身` 均被低估**，
> §3.4 的三个往返均被高估。

#### 变动三：节点区间延伸到「响应真正写出」，并按点位去重

两件事叠加：

1. **下游回写此前测不到。** 与变动二同因 —— 下游 `write()` 也只是入队，
   真正的 writev 在 `rpc_done` 之后执行，那时探针绑定已被释放。
   补齐后实测 envoy-out **11.2 µs**、envoy-in **20.2 µs**。
2. **`onWriteReady` 每条 trace 会触发多次**（第 1 次是真正发送，之后是缓冲区
   已排空的空写）。分析工具原先用**全部事件**取节点区间的 min/max，被空写撑长；
   而各阶段用的是**每个点位最早一次** —— 两处口径不一致。现已统一为后者。

> 影响：**§3.2 的两个 Envoy 自身被低估**（各约 11~20 µs），
> §3.4 的跨机往返与 client↔envoy-out 往返被高估。

#### 净效果与一个外部旁证

在 2026-08-10 的数据上，三次修正之后：

| | 修正前 | 修正后 |
|---|---:|---:|
| kitex-client 自身 | 13.9 µs | **20.7 µs** |
| UDS 往返 client↔envoy-out | 19.3 µs | **4.7 µs** |
| envoy-out 自身 | 24.5 µs | **44.3 µs** |
| 跨机往返 | 108.4 µs | **68.8 µs** |
| envoy-in 自身 | 43.9 µs | **68.0 µs** |
| UDS 往返 envoy-in↔server | 34.7 µs | **5.4 µs** |
| kitex-server 自身 | 19.2 µs | **32.1 µs** |

**四个「自身」全部上升，三个「往返」全部下降** —— 因为三次修的是同一类偏差：
**高估链路、低估节点**。

> 上表跨了不同轮次的数据，而本环境轮间噪声是 8~10 %（§2.3）。
> 个位数微秒的差异不要读成口径变化，真正由口径造成的是那几个成倍的变化。

**外部旁证**：修正后的跨机往返 **68.8 µs**，而两机 `ping` 实测 RTT
**0.068 ms = 68 µs**（§1）。往返里现在只剩纯网络，这个吻合是这一串修正
正确性最强的独立验证 —— 旧口径那 108.4 µs 里有约 40 µs 根本不是网络。

**本报告的结论是否受影响**：§0 的五条结论、§3.2 的级差（「加一个 sidecar 多少钱」）
与 §6 的机器差异对照**都不受影响** —— 它们比较的是同一口径下的**相对量**，
三级/两跳用的是同一套定义，偏差同向抵消。受影响的是**绝对分解**：
「往返占多少、节点自身占多少」这类读法要按上面的方向校正。
...
**这个定义在 2026-08-10 修正过三次，下表用的是旧口径，读数前先看 §2.6。**
...
> 下面的「往返」在旧口径下**被高估**，其中混着两端各自的本地开销。见 §2.6。
