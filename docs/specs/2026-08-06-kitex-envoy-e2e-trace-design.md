...
**本环境的实测扫描结果**(跨机双跳,payload 64 B,trace 关闭):
...
| 128 | **78,037 ↓** | 1.324 ms | 5.498 ms | **×0.93 吞吐倒退** |

**QPS_max ≈ 84 k(c=64)。** c=128 时吞吐反而下降、延迟翻倍,是典型的过饱和。

据此,**归因测量固定在 c=16(约 43 k QPS,约 51% 饱和点)** —— 该档 QPS 仍随并发近线性增长(×1.73),排队尚未主导端到端延迟。

报告中的每一组归因数据都必须标注其运行 QPS 与当时的 QPS_max,否则读者无法判断该数据是否落在非饱和区。

### 8.5.1 打点开销的构成(实现后的实测分析)

探针实现完成后,对开销来源做了逐项拆解。**结论与直觉不同:系统调用不是最大的一项。**

| 来源 | 当前实现 | 说明 |
|---|---|---|
| **时钟读取** | 每个点 **2 次** `clock_gettime` | 宏同时取 `monotonicTime()` 与 `systemTime()`。Linux 上走 vDSO,各约 20–25 ns。8 个点 = 16 次 ≈ **350 ns/请求** |
| **堆分配** | 每个点 **1 次** | `Event.trace` 是 32 字符的 trace-id,超过 SSO 阈值必然堆分配。8 点 = 8 次 ≈ **300 ns/请求** |
| **write 系统调用** | 每个采样请求 **1 次** | `endRpc` 里无条件 `fflush`,约 1–2 µs |
| 哈希查找 | 每个点 1 次 | 查 binding 判采样,约 100 ns/请求 |

**未采样请求**不受以上影响,只付「一次 enabled 判断 + 一次哈希查找 + 一次布尔判断」≈ 10–20 ns。

#### 为什么会有「每请求一次 fflush」

原设计是纯内存缓冲(攒够 512 条或 200 ms 才落盘)。但踩了一个坑:**流量停止后最后一批事件永远刷不出去**,短测试跑完根本没有 trace 文件,看起来像探针完全没工作。当时用「每 RPC 刷一次」绕过,那是权宜之计。

正确做法是保留内存缓冲,另行解决收尾:阈值 + 时间触发 + **进程退出时 flush**。在 1/100 采样、10 万 QPS 下,每秒仅约 16 次 write,可忽略。

#### 三项可消除的开销

1. **时钟读取减半** —— `wall_ns` 只用于粗排序与人眼可读,精确计算全靠 `mono_ns`。每个 RPC 只在 bind 时读一次 wall,其余点由 mono 差值推算。
2. **消除每点的堆分配** —— trace-id 在一次 RPC 内不变,存一份在 binding 里,事件只存引用。
3. **去掉每请求的 fflush** —— 见上。

**这些数字是估算,不是实测。** 因此采取「先按现状跑一轮对照 → 优化 → 再跑一轮」的顺序,用前后两组真实数据说话,而不是拿估算当结论。
...
| 组 | 配置 | 隔离出什么 |
|---|---|---|
| 基线 | 不设 `KITEX_PROBE_PATH`,探针不激活 | 无插桩的真实性能 |
| A | 探针激活,采样率 0 | **插桩存在但不采样的固定成本**(分支、哈希查找、代码膨胀) |
| B | 采样率 1/100 | 实际归因配置的成本 |
| C | 采样率 100% | 成本上界 |

`基线 → A` 的差值是"哪怕不采样也要付出的代价",这个数字决定了插桩能否长期留在生产构建里。

#### 8.6.1 单次运行不足以下结论:噪声可能吞掉效应量

首轮四组对照(各跑一次,c=16)的结果:

| 组 | QPS | p50 | p99 |
|---|---|---|---|
| 基线 | 40,945 | 366 µs | 944 µs |
| A 探针开/采样 0 | **42,944** | 351 µs | 753 µs |
| B 探针开/采样 1% | 39,765 | 372 µs | 845 µs |
| C 探针开/采样 100% | 35,859 | 405 µs | 1.354 ms |

**A 组比基线快 4.9%。** 加插桩不可能使系统变快,所以这只说明一件事:**单次运行的噪声已经超过了 A 组要测的效应量**。

由此确立两条规则:

1. **每组重复至少 5 次**,报告中位数与 [最小, 最大] 区间。
2. **若某组的区间与基线区间重叠,则不得宣称存在开销** —— 只能说"该效应低于本方法的噪声下限"。

只有 C 组(100% 采样)的效应明确超出噪声:吞吐 −12.4%、p99 +43%。这也从另一面印证了 §8.4 的判断:**全量采样在压测下不可用**。
