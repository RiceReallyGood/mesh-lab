...
## 2026-08-10：三个区间在本目录里被改过名

本目录本应是「未经加工的 merge 输出」，但 2026-08-10 发现三个区间的名字与它们
在源码里包住的代码对不上，会误导读者。**改了名字，一个数字都没动**：

| 原名 | 现名 | 为什么原名是错的 |
|---|---|---|
| `★等待请求体` / `★等待响应体` | `payload反序列化` | 它包住的是 `unmarshalThriftData`，是**解码**不是等待 |
| `★等待对端(纯网络)`（**仅 kitex-server 栏**） | `取首字节(Peek)` | 服务端 `OnRead` 触发时数据已在 LinkBuffer，`Peek` 立即返回，**没有网络等待** |
| `payload解码` | `payload编码` | `mesh_payload_codec_*` 只插在 `encodePayload` 前后，**只覆盖发送路径** |

`kitex-client` 栏的 `★等待对端(纯网络)` **保持原样** —— 客户端那次 `Peek` 是真的
在阻塞等响应（217.7 µs），叫「等待对端」没问题。

详细依据见 [`../../probe-points.md`](../../probe-points.md) 的「三个名字骗人的区间」。

> **本目录的文件没有用新版 merge 重新生成过** —— 当时的原始 `.ndjson` 未入库
> （`.gitignore` 排除），无法重跑。所以这些文件是「2026-08-07 的数值 + 2026-08-10 的
> 术语」，除标签外的排版一律保持原样（字节列位置逐行核对过，未偏移）。
> 也因此，本目录的**区间顺序仍是旧的声明顺序**，不是新版按真实时序排的顺序。

> **格式统一没有回溯到本目录。** 2026-08-10 把 merge 的 detail 标签统一成了树形缩进
> （去掉 ①→② 这类带圈编号、`↳`、两空格缩进等混排）。本目录仍是旧格式 ——
> 那次改的是**呈现形式**，不影响任何数值与语义，没必要再动一遍归档文件。
> 上面那三条改名则不同：改的是**含义错误**，所以回溯了。
